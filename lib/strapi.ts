...
import type { Author, Article, Tag, Pillar, PillarName, ArticleStatus, Category } from '@/types';
...
export function transformArticle(strapiArticle: StrapiData<StrapiArticleAttributes>): Article {
    ...
    const category: Category = attrs.category?.data
        ? {
            id: attrs.category.data.id.toString(),
            name: attrs.category.data.attributes.name,
            slug: attrs.category.data.attributes.slug,
            subtitle: '',
            description: attrs.category.data.attributes.description || '',
            details: [],
            order: 0,
        }
        : {
    ...
    return {
        ...
        pillar,
        category,
        subcategories: [], // Populate if needed
        tags,
        author,
