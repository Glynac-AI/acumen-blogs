...
export async function generateMetadata({ params }: BlogPageProps): Promise<Metadata> {
    ...
    const article = await getArticleBySlug(slug); // ✅ Fetch from Strapi
...
export default async function BlogArticlePage({ params }: BlogPageProps) {
    ...
    const allArticles = await getArticles(); // ✅ Fetch all from Strapi
    const relatedArticles = allArticles
        .filter(a => a.pillar.id === article.pillar.id && a.id !== article.id)
        .slice(0, 3);

    const pillarSlug = pillarToSlug(article.pillar);
                        ...
                        <div className="flex items-center justify-between border-t border-b border-gray-200 py-6">
                            <div className="flex items-center space-x-4">
                                <div className="relative w-14 h-14 rounded-full overflow-hidden border-2 border-[#49648C]">
                                    <Image
                                        src={article.author.photo || 'https://placehold.co/400x400/49648C/FFFFFF?text=A'}
